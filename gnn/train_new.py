...
# dataname='imdb4'
result=open('../p_results/result-{}-{}-{}-{}-{}.txt'.format(dataname, kernel_size, inception_depth, attention_size, train_ratio),'a')
...
for numi in range(int(iternum)):
    ...
    for step in range(epochs):
        ...
        if max_acc < accuracy_class:
		max_f1 = fscore_sa
		max_acc = accuracy_class
		max_step = step
	# accuracy_multiclass_for_each(truelabels[testindex], testlabels[testindex])
        ...
        if step == epochs - 1:
            result.write(
                str(numi) + ':' + str([max_step, max_acc, max_f1]) + '\n')
